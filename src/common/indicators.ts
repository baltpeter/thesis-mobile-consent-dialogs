...
export const button_text_fragments = {
    clear_affirmative: normalizeFragments([/(accept|agree|allow|consent|permit) and continue/, 'accept', 'agree', 'allow', 'consent', 'permit', /(select|choose) all/, 'yes', /(akzeptieren?|zustimmen|zulassen|annehmen|erlauben?|einwilligen|genehmigen?) und weiter/, /akzeptieren?/, 'zustimmen', 'zulassen', 'annehmen', /erlauben?/, 'einwilligen', /genehmigen?/, /alle (aus)?wählen/, /stimm[^.]{0,4} zu/, /nehm[^.]{0,4} an/, /willig[^.]{0,4} ein/, 'ja'], true),
    ...
    clear_negative: normalizeFragments([...clear_negative_words, /(do not|don't|without) (accept|agree|allow|consent|permit)(ing)?/, /no,? thanks?( you)?/, 'i want to opt out', /nicht (akzeptieren?|zustimmen|zulassen|annehmen|erlauben?|einwilligen|genehmigen?)/, /ohne (akzept|zustimm|zulass|annehm|erlaub|einwillig|genehmig)/, /lehn[^.]{0,4} ab/, /nein,? danke/], true),
    hidden_negative: normalizeFragments([/customi(z|s)e/, /personali(z|s)e/, /more (choices|details|info|information)/, 'settings', 'options', 'preferences', 'configure', /(adjust|change|manage|view|show|more)[^.]{0,12} (details|settings|options|preferences|cookies|choices)/, /(confirm|save)[^.]{0,8} selection/, 'later', 'skip', 'exit', 'cancel', /(learn|read) more/, 'not now', 'no, see ads that are less relevant', 'anpassen', 'personalisieren', 'einstellungen', 'einstellen', 'konfigurieren', 'optionen', /(details|einstellungen|optionen|cookies|auswahl)[^.]{0,5} (anpassen|ändern|verwalten)/, /mehr (details|infos|information)/, 'details anzeigen', 'anpassen', /auswahl (bestätigen|speichern)/, 'später', /schlie(ß|ss)en/, 'beenden', 'abbrechen', /mehr (erfahren|lesen)/, 'jetzt nicht', 'überspringen'], true),
};
// prettier-ignore
