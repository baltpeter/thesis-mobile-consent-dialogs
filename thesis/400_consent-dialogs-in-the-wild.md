...
On a technical level, the TCF mainly regulates two aspects. For one, it mandates that CMPs store consent records in a so-called *TC string*, which encodes^[IAB Europe offers a JavaScript library (<https://github.com/InteractiveAdvertisingBureau/iabtcf-es>) and an online tool (<https://iabtcf.com>) to decode and encode TC strings.] the following information [@iabtechlabTransparencyConsentString2022]:
...
If a script then wants to start processing, it has to [@iabtechlabConsentManagementPlatform2021]:
...
For those consent dialogs that _do_ implement the TCF, we still extract the data from `NSUserDefaults` or `SharedPreferences` and perform an analysis on that.
